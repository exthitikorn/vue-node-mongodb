      ...
      <table id="ofcccs" class="ui celled compact table">
        ...
        <tr v-for="(ofccc, i) in ofcccs" :key="i">
          ...
          <td width="150" class="center aligned">
            <router-link class="ui green button" :to="{ name: 'show-ofccc-sdp', params: { id: ofccc._id } }"
              >Show SDP</router-link>
          </td>
          <td width="75" class="center aligned">
            <router-link class="ui orange button" :to="{ name: 'edit-ofccc', params: { id: ofccc._id } }"
              >Edit</router-link>
          </td>
          <td width="75" class="center aligned" @click.prevent="onDestroy(ofccc._id)">
            <a class="ui red button" :href="`/ofcccs/${ofccc._id}`">Delete</a>
          </td>
        </tr>
      </table>
    </div>
